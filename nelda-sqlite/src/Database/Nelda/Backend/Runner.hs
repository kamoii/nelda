...
module Database.Nelda.Backend.Runner where
...
type QueryRunner a = Sql -> [SqlParam] -> IO a
...
runStmt :: Connection -> Sql -> [SqlParam] -> IO (Int, [[SqlValue]])
...
sqliteQueryRunner db (Sql qry) params = do
    eres <- try $ do
      stm <- prepare db qry
